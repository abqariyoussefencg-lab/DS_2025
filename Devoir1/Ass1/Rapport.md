...
* **Comment :** La collecte s'est faite par deux moyens :
   1. **Questionnaires :** Les étudiants ont rempli des questionnaires pour fournir les données démographiques, sociales et liées à leur mode de vie (ex: `studytime`, `goout`, `Dalc`, `Walc`, `famsup`, etc.).
   2. **Registres scolaires :** Les données objectives comme les notes (`G1`, `G2`, `G3`), les absences (`absences`) et les échecs passés (`failures`) ont été extraites des bases de données de l'école.
...
1. **`failures` (Échecs passés) :** C'est le facteur négatif le plus puissant. Un étudiant qui a déjà échoué à des cours dans le passé est massivement plus susceptible d'échouer à nouveau.
2. **`higher` (Veut aller à l'université) :** L'ambition personnelle était un prédicteur positif très fort. Les étudiants qui ont répondu "oui" (`yes`) à vouloir poursuivre des études supérieures avaient tendance à avoir de bien meilleures notes, indépendamment d'autres facteurs.
3. **`Medu` & `Fedu` (Éducation des parents) :** Le niveau d'éducation de la mère (`Medu`) et du père (`Fedu`) était un indicateur important. Des parents ayant un niveau d'études supérieur étaient corrélés à de meilleurs résultats pour l'étudiant.
4. **`school` (L'école) :** L'école fréquentée (GP ou MS) avait un impact notable, suggérant qu'une école était globalement plus performante que l'autre.
5. **`goout` (Sorties avec les amis) :** Un niveau élevé de sorties (`goout` = 4 ou 5) était fortement corrélé à de moins bonnes notes.
...
1. La réussite scolaire est fortement auto-corrélée (les bonnes notes amènent les bonnes notes).
2. En l'absence de notes, une combinaison de facteurs comportementaux (`failures`, `higher`, `goout`) et socio-démographiques (`Medu`, `school`) peut créer un modèle de machine learning (type Arbre de Décision ou SVM) capable d'identifier les étudiants "à risque" avec une bonne précision.
